...
export default function PostsList (props){
	...
	const createList = function() {
		const list = [];
		articles.map((article, i) => {list.push(<PostEntry id={i} className="post-list-entry" title={article.title} publication_date={article.publication_time} description={article.description} id={article.shortTitle} key={article.id}/>)})
		return list;
	}
	...
	if (error) {
	  ...
	  return(
	  		<>
				{!isLoadedList ? <Loading /> : createList()}
	  		</>);
  	}
}